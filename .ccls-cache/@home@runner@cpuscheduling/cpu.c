...
pthread_mutex_t main_thread = PTHREAD_MUTEX_INITIALIZER;
...
void en_q(int flag_q, int priority, int burst, int *cpu_IO, char *sched_alg) {
  ...
  new_node->next = NULL;
  new_node->prev = NULL;
  gettimeofday(&new_node->start_time, NULL);
  printf("start time --------- %ld", new_node->start_time.tv_usec / 1000);
  usleep(50 * 1000);
  gettimeofday(&new_node->end_time, NULL);
  printf("total time -----------%ld\n", (new_node->end_time.tv_usec - new_node->start_time.tv_usec) / 1000);

  printf("enq cpu io %d\n", new_node->cpu_IO[0]);
  // checks if first node

  if (flag_q == 0) { // queue is for cpu bursts
    if (ready_head == NULL) {
      ready_tail = new_node;
      ready_head = new_node;
    } else {
  ...
  } else if (flag_q == 1) { // queue is for io bursts
    if (io_head == NULL) {
      io_head = new_node;
      io_tail = new_node;
    } else {
...
void cpu_thread() {
  if (ready_head != NULL) {
    ...
    en_q(1, dq->priority, dq->burst_number, dq->cpu_IO, "");
    free_nodes(dq);
    dq = NULL;
  }
}
...
int main(int argc, char *argv[]) {
  ...
  if (strcmp(argv[1], "-alg") == 0) {
    ...
    // pthread_create(&thread_cpu, NULL, cpu_thread, NULL);
